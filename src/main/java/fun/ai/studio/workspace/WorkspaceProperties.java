...
public class WorkspaceProperties {
     ...
     * - APP（推荐）：将 npm cache 放到 workspace 根目录下按 appId 隔离的目录（容器内通常是 /workspace/.npm-cache/{appId}），
     *   避免落在项目目录内被 Vite/Chokidar watcher 扫描触发 ENOSPC；平台在删除应用时会一并清理。
     *   兼容迁移：旧版本可能为 {APP_DIR}/.npm-cache，受控任务启动时会自动迁移/清理。
