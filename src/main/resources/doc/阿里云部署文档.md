...
<script>setTimeout(function(){location.reload();},5000);</script>
    ...
    # ===== 1) /ws/{userId}/... 反代到 127.0.0.1:{hostPort}（剥离 /ws/{userId} 前缀后再转发）=====
    location ~ ^/ws/(?<uid>\d+)(?<rest>/.*)?$ {
        ...
        # 重要：设置 cookie 记录当前 ws_uid（用于处理 Vite HMR 的绝对路径资源，如 /@vite/client）
        # 场景：有些全栈项目（或 Vite middleware）会在 HTML 里引用 "/@vite/client"（不带 /ws/{uid} 前缀）。
        # 若不做此 cookie 路由，浏览器会请求 http://host/@vite/client，命中后端 API（可能 401/404）。
        #
        # 【iframe 跨站注意】如果预览页被“控制台域名”用 iframe 嵌入（top-level site 与 /ws 域名不同），
        # 浏览器会把 ws_uid 视为第三方 Cookie：SameSite=Lax/Strict 默认不会发送，导致根路径资源与 /api 分流失效。
        # 这时必须：
        ...
        #
        # HTTP:
        # add_header Set-Cookie "ws_uid=$ws_uid; Path=/; SameSite=Lax" always;
        # HTTPS:
        add_header Set-Cookie "ws_uid=$ws_uid; Path=/; SameSite=None; Secure" always;

        # 关键：不要剥离 /ws/{userId} 前缀。
        # 上游 dev server 很常见会把 "/" 302 到 "/ws/{userId}/"；若剥离前缀会造成浏览器 302 循环，预览打不开。
    ...
    # - /src/main.tsx
    # - /@react-refresh
    # 这些请求不带 /ws/{uid} 前缀，会绕过上面的 /ws location，若直接落到后端 API 常见表现为 401/404。
    #
    # 方案：在首次访问 /ws/{uid}/ 时写入 cookie ws_uid，然后这里用 cookie 反查端口并转发到对应 workspace。
    location ~ ^/(?:@vite/|src/|@react-refresh$|__vite_ping$) {
        set $ws_uid $cookie_ws_uid;
        if ($ws_uid = "") { return 404; }
    ...
    #
    # 思路：当浏览器已访问过 /ws/{uid}/（我们会设置 cookie ws_uid），则对“非 /api 的其它根路径请求”
    # 自动按 cookie ws_uid 路由到该用户 workspace。
    #
    ...
    location / {
        ...
        # 如果存在 ws_uid cookie，则把该请求交给 @ws_root_proxy
        if ($cookie_ws_uid ~ "^\d+$") { return 418; }
    ...
    location @ws_root_proxy {
        ...
        # 根路径资源（/@vite、/src、/assets...）也需要落到 /ws/{userId}/ 前缀下，避免上游再次重定向或 404。
        proxy_pass http://127.0.0.1:$ws_port/ws/$ws_uid$request_uri;
    }
