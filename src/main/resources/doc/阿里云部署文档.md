...
- **Verdaccio（推荐）**：同机 npm registry 代理仓库（缓存包与元数据，供 workspace 容器通过 `funai-net` 访问）
...
funai.workspace.previewPathPrefix=/preview
...
# ===== npm cache（最简方案，可选）=====
# 跨用户复用 npm 下载缓存，减少 npm install 时间与失败率；后续可升级为 Verdaccio/Nexus 私有仓库

# ===== Workspace Mongo（可选）=====
# 一个用户容器仅一个 mongod；dbPath 为用户级固定目录（bind mount 持久化），每个项目用 dbName 隔离：
# dbName = {dbNamePrefix}{appId}（例如 db_2002）
...
## 7. Nginx 配置（API + /preview 预览反代 + 友好 502）

目标：

- 外部只访问 `80/443`
- `/` 反代到后端 `127.0.0.1:8080`
- `/preview/{appId}/...` 反代到对应用户 workspace 的 `hostPort`
...
body{margin:0;background:#0b1220;color:#e8eefc;font-family:system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial,"PingFang SC","Microsoft YaHei",sans-serif}
.wrap{min-height:100%;display:flex;align-items:center;justify-content:center;padding:32px}
.card{width:min(860px,100%);background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.12);border-radius:16px;box-shadow:0 20px 80px rgba(0,0,0,.45);padding:28px}
.h{font-size:22px;font-weight:700;margin:0 0 8px}
.sub{opacity:.85;margin:0 0 18px;line-height:1.6}
.grid{display:grid;grid-template-columns:1fr;gap:16px}
.box{background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.10);border-radius:12px;padding:14px 16px}
.k{font-weight:650;margin:0 0 8px}
ol{margin:0;padding-left:18px;line-height:1.75;opacity:.92}
code{background:rgba(255,255,255,.10);padding:2px 6px;border-radius:8px}
.row{display:flex;gap:10px;flex-wrap:wrap;margin-top:14px}
.btn{display:inline-flex;align-items:center;justify-content:center;padding:10px 14px;border-radius:10px;border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.10);color:#e8eefc;text-decoration:none}
.btn:hover{background:rgba(255,255,255,.16)}
.tip{margin-top:10px;font-size:12px;opacity:.75}
</style></head>
<body><div class="wrap"><div class="card">
<h1 class="h">工作区已休眠</h1>
<p class="sub">当前用户容器存在，但预览服务（例如 Vite dev server）未在监听端口，因此暂时无法访问预览页面。为节省资源，系统会在空闲时自动暂停运行。</p>
<div class="grid">
  <div class="box">
    <div class="k">你可以这样恢复</div>
    <ol>
      <li>回到 Fun AI Studio 的应用/工作区页面，点击「启动/恢复/预览」</li>
      <li>等待 5-15 秒，本页将自动刷新</li>
      <li>如果一直无法恢复，请查看 <code>/api/fun-ai/workspace/run/status</code> 返回的 message（或 workspace-dev 日志）</li>
    </ol>
    <div class="row">
      <a class="btn" href="javascript:location.reload()">立即刷新</a>
      <a class="btn" href="/">返回首页</a>
    </div>
    <div class="tip">提示：该页面由 Workspace 开发服务器（大机）Nginx 兜底返回，并不代表接口鉴权失败。</div>
  </div>
</div>
<script>setTimeout(function(){location.reload();},5000);</script>
    ...
    # ===== 1) /preview/{appId}/... 反代到 127.0.0.1:{hostPort}（剥离 /preview/{appId} 前缀后再转发）=====
    location ~ ^/preview/(?<appid>\d+)(?<rest>/.*)?$ {
        # 子请求要用到 appId（注意：子请求会继承该变量）
        set $ws_appid $appid;
        ...
        # 重要：设置 cookie 记录当前 ws_appid（用于处理 Vite HMR 的绝对路径资源，如 /@vite/client）
        # 场景：有些全栈项目（或 Vite middleware）会在 HTML 里引用 "/@vite/client"（不带 /preview/{appId} 前缀）。
        # 若不做此 cookie 路由，浏览器会请求 http://host/@vite/client，命中后端 API（可能 401/404）。
        #
        # 【iframe 跨站注意】如果预览页被“控制台域名”用 iframe 嵌入（top-level site 与 /preview 域名不同），
        # 浏览器会把 ws_appid 视为第三方 Cookie：SameSite=Lax/Strict 默认不会发送，导致根路径资源与 /api 分流失效。
        # 这时必须：
        # - 使用 HTTPS（否则 Secure Cookie 不会生效）
        # - 设置 SameSite=None; Secure
        #
        # 下面给出两种写法：
        # - HTTP（非 iframe / 临时调试）：Lax
        # - HTTPS（iframe 跨站）：None + Secure
        #
        # HTTP:
        # add_header Set-Cookie "ws_appid=$ws_appid; Path=/; SameSite=Lax" always;
        # HTTPS:
        add_header Set-Cookie "ws_appid=$ws_appid; Path=/; SameSite=None; Secure" always;

        # 关键：剥离 /preview/{appId} 前缀后转发
        # - /preview/10000023/        -> /
        # - /preview/10000023/@vite/* -> /@vite/*
        # - /preview/10000023/api/*   -> /api/*
        set $rest_path $rest;
        if ($rest_path = "") { set $rest_path "/"; }
        proxy_pass http://127.0.0.1:$ws_port$rest_path;
    }

    # ===== 1.1) Vite/前端开发资源（可选，但强烈建议加上）=====
    # 场景：有些项目会在 HTML 里引用绝对路径资源，例如：
    # - /@vite/client
    # - /src/main.tsx
    # - /@react-refresh
    # 这些请求不带 /preview/{appId} 前缀，会绕过上面的 /preview location，若直接落到后端 API 常见表现为 401/404。
    #
    # 方案：在首次访问 /preview/{appId}/ 时写入 cookie ws_appid，然后这里用 cookie 反查端口并转发到对应 workspace。
    location ~ ^/(?:@vite/|src/|@react-refresh$|__vite_ping$) {
    ...
    # ===== 1.2) （推荐增强）根路径资源自动路由到 workspace（避免不断补漏）=====
    # 背景：Vite dev / 某些全栈项目会在 HTML 中引用大量根路径资源（/src/*、/assets/*、/node_modules/*、/vite.svg...）。
    # 如果只靠 1.1 逐个补前缀，很容易漏掉，表现为 401/404。
    #
    # 思路：当浏览器已访问过 /preview/{appId}/（我们会设置 cookie ws_appid），则对“非 /api 的其它根路径请求”
    # 自动按 cookie ws_appid 路由到该用户 workspace。
    #
    # 注意：为避免在 if 内直接 proxy_pass，这里使用 error_page 418 技巧。
    location / {
        # /api/ 仍然走后端（你也可以用单独的 location ^~ /api/ 覆盖）
        if ($request_uri ~* "^/api/") { proxy_pass http://127.0.0.1:8080; break; }

        # 如果存在 ws_appid cookie，则把该请求交给 @ws_root_proxy
        if ($cookie_ws_appid ~ "^\d+$") { return 418; }

        # 默认：站点/API 走 Spring Boot
    ...
    location @ws_root_proxy {
        ...
        # 根路径资源（/@vite、/src、/assets...）直接转发给上游（上游以 / 为根工作）
        proxy_pass http://127.0.0.1:$ws_port$request_uri;
    }
    ...
    location = /_ws_port {
        ...
        proxy_pass http://127.0.0.1:8080/api/fun-ai/workspace/internal/nginx/port?appId=$ws_appid;
...
curl -I http://127.0.0.1/preview/20000021/
curl -I http://47.118.27.59/preview/20000021/
```
...
### 8.2 元数据与端口分配（对齐 Nginx /preview 反代）
...
- **Nginx 端口查询（关键）**：
  - Nginx `/preview/{appId}/...` 会通过 `auth_request` 调用内部接口查询端口：
...
- **启动 dev（非阻塞）**：`POST /api/fun-ai/workspace/run/start?userId=...&appId=...`
  ...
  - dev server 参数包含：`--host 0.0.0.0 --port {containerPort} --strictPort --base /preview/{appId}/`
...
- **查询运行态**：`GET /api/fun-ai/workspace/run/status?userId=...`
  ...
  - `RUNNING` 时返回 `previewUrl = {previewBaseUrl}{previewPathPrefix}/{appId}/`
