# 部署手册：阿里云 Linux 3（单机版 Workspace + Nginx 仅开放 80/443）
...
- 每个用户一个 workspace 容器（`ws-u-{userId}`）
- 每个用户有持久化目录（宿主机）：`{hostRoot}/{userId}/`
- 应用代码目录以 `appId` 为稳定目录名：`{hostRoot}/{userId}/apps/{appId}`
- 开发运行状态写入：`{hostRoot}/{userId}/run/current.json`、`dev.pid`、`dev.log`
- 支持省钱模式：空闲自动 stop run / stop container
...
- **启动 dev（非阻塞）**：`POST /api/fun-ai/workspace/run/start?userId=...&appId=...`
  - 写入 `current.json(pid=null)` → 后台脚本安装依赖 + 启动 Vite
  - Vite 启动参数包含 `--base /ws/{userId}/` 以适配 Nginx 路径反代

- **查询运行态**：`GET /api/fun-ai/workspace/run/status?userId=...`
  - 状态：`IDLE / STARTING / RUNNING / DEAD / UNKNOWN`
  - `RUNNING` 时返回 `previewUrl`（建议配置为 `http(s)://<domain>/ws/{userId}/`）
...
- `idleStopRunMinutes`：超过则 stop run
- `idleStopContainerMinutes`：超过则 stop container

> 约定：配置 `<=0` 表示禁用（避免误配导致“立刻回收”）。
