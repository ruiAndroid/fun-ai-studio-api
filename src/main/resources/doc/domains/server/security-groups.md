...
## 1. 当前在线服务器清单（现网 7 台 + Mongo 待分配）

| 序号 | 角色 | 内网 IP | 主要端口 |
|------|------|---------|----------|
| 1 | API（统一入口 / Control Plane） | `172.21.138.91` | 80/443, 8080 |
| 2 | Agent Node（Agent 服务） | `172.21.138.88` | 80 |
| 3 | Workspace-dev（workspace-node + Nginx） | `172.21.138.87` | 80, 7001 |
| 4 | Deploy（Deploy 控制面） | `172.21.138.100` | 7002 |
| 5 | Runner（执行面） | `172.21.138.101` | - |
| 6 | Runtime（runtime-agent + 网关） | `172.21.138.102` | 80/443, 7005 |
| 7 | Git + Harbor（Gitea + 镜像仓库） | `172.21.138.103` | 3000, 2222, 80（HTTP） |
| 8 | 运行态 Mongo | `待分配` | 27017 |
...
- **Agent Node（88）**：`80`（Agent 服务；通常由 91 的 Nginx 统一转发 `/fun-agent/**`）
- **监控（Prometheus 抓取）**
...
### 3.2 Agent Node（172.21.138.88）

- **入站（Inbound）**
  - **TCP 80**：允许来源 **`172.21.138.91/32`**
    - 用途：API(91) 上 Nginx → Agent(88)（转发 `/fun-agent/**`）
  - （监控）**TCP 9100**：允许来源 **`172.21.138.91/32`**
    - 用途：API(91) 上 Prometheus → Agent(88) node_exporter

- **出站（Outbound）**
  - 默认放开即可（若 Agent 需要访问外部 LLM/依赖服务，再按需收敛）

### 3.3 Workspace-dev（172.21.138.87）
...
- **出站（Outbound）**
  - **TCP 80** → **`172.21.138.88/32`**
    - 用途：API(91) 上 Nginx → Agent(88)（`/fun-agent/**` 反代）
...
> 若你们现网包含 Agent Node（88），则上面的监控出站 `9100` 也应追加 `172.21.138.88/32`。

### 3.5 Runner（172.21.138.101）
